...
namespace DamageNumbersPro
{
    public class DNP_DemoManager : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (Input.GetMouseButton(1) && Time.time > lastSpawnTime + 0.05f)
            {
                SpawnNumber();
        ...
        void SpawnNumber()
        {
            ...
            DamageNumber dn = numberPrefabs[currentIndex].CreateNew(number, position); //Creating a new Damage Number from Prefab.
            ...
            if (hit.collider != null || hit3D.collider != null)
            {
                ...
                {
                    if(hit3D.collider.GetComponent<DNP_SineMover>() != null)
                    {
        ...
        void UpdateCurrent()
        {
            currentText.text = "<color=#FFFFFF99>Current:</color> " + numberPrefabs[currentIndex].name;
        }
    }
