...
public class DS_BoltActionSniperScriptLPFP : MonoBehaviour {
	...
	private void Update () {
		...
		//Shoot
		if (Input.GetMouseButton (0) && !outOfAmmo && !isReloading && !isInspecting && !isRunning && Time.time >= nextTimeToFire) 
		{
			if (!isReloading) 
			{
				lastFired = Time.time;
